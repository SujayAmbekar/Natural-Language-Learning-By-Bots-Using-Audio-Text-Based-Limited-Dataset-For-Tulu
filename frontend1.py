...
def start_button():
    ...
    tk.Button(start_window, text="Enter Sentence",fg = "black",bg='white',font = "Verdana 26 bold", command=enterSent_button).pack(pady=5)
    tk.Button(start_window, text="Generate Sentence",fg = "black",bg='white',font = "Verdana 26 bold", command=generateSent_button).pack(pady=5)
    tk.Button(start_window, text="Translate Sentence",fg = "black",bg='white',font = "Verdana 26 bold", command=translateSent_button).pack(pady=5)
...
def enterSent_button():
    ...
    tk.Label(enter_window, text = "Enter sentence.", fg = "black", font = "Verdana 27 bold").pack(pady = 20, side = TOP, anchor = 'w')
    ...
    tk.Button(enter_window,text = 'Submit',fg = "blue",bg='white',font = "Verdana 20 bold", command = enterSent_submit).pack(pady=60)
...
'''def enterSent_getdetails(engw, pos, app):
    entergd_window = Toplevel(window)
    entergd_window.title("Tulu Translator- Enter Sentence")
    entergd_window.geometry("1400x800")
    tk.Label(entergd_window).pack()
    tw = tk.StringVar()
    gender = tk.StringVar()
    tk.Label(entergd_window, text="What does "+engw+" translate to in the given sentence if POS is "+pos+" ?", fg = "black", font = "Verdana 27 bold").pack(pady = 20, side = TOP, anchor = 'w')
    tk.Entry(entergd_window,textvariable = tw, font=('Verdana',20,'normal')).pack(pady=35)
    tk.Label(entergd_window, text="What is the gender?", fg = "black", font = "Verdana 27 bold").pack(pady = 20, side = TOP, anchor = 'w')
    tk.OptionMenu(entergd_window, gender, "masculine", "feminine", "neuter", "NULL")

    return tw.get(), gender.get()'''
...
#    text.pack()
    ...
    cbutton=tk.Button(generateSent_window,image=correct,borderwidth=0,command=lambda m='correct': which_button(m, generateSent_window)).pack(padx= 70, pady= 80, side=LEFT, anchor='w')
    qbutton=tk.Button(generateSent_window,image=questionmark,borderwidth=0,command=lambda m='questionmark': which_button(m, generateSent_window)).pack(padx= 70, pady= 80, side=LEFT, anchor='center')
    ibutton=tk.Button(generateSent_window,image=incorrect,borderwidth=0,command=lambda m='incorrect': which_button(m, generateSent_window)).pack(padx= 70, pady= 80, side=LEFT, anchor='e')

    generateSent_window.mainloop()

def which_button(button_press, x): #dispays on the terminal which button - correct/question mark/incorrect was clicked
    print(button_press)
    global posval
...
def translateSent_button():
    ...
    tk.Button(translateSent_window, text="OK",fg = "blue",bg='white',font = "Verdana 26 bold", command=display_input).pack(anchor='center', pady=5)
...
tk.Button(window, text="Quit",fg = "blue",bg='white',font = "Verdana 26 bold", command=quit_button).pack()
